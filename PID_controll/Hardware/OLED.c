...
void OLED_TEST()
{
	...
	for(uint8_t i = 0;i <= 7;i ++)
	{
		...
		{
			sendBuffer[j+1] = OLED_DisplayBuf[i][j];
		}
		OLED_SendCmd(0xB0 + i);
		OLED_SendCmd(0x00);
		OLED_SendCmd(0x10);
...
void OLED_WriteData(uint8_t *Data, uint8_t Count)
{
	...
	/*for (i = 0; i < Count; i ++)
	{
		OLED_SendCmd(Data[i]);	//依次发送Data的每一个数据
	}*/
	for(i = 128;i > 0;i --)
	{
		Buffer[i] = Data[i - 1];
	...
	Buffer[0] = 0x40;
	HAL_I2C_Master_Transmit(&hi2c1,OLED_ADDRESS,Buffer,Count,HAL_MAX_DELAY);
}
...
void OLED_Update(void)
{
	...
	for (j = 0; j < 8; j ++)
	{
		...
		OLED_WriteData(OLED_DisplayBuf[j], 129);
	}
}
...
void OLED_ClearArea(int16_t X, int16_t Y, uint8_t Width, uint8_t Height)
{
	...
	for (j = Y; j < Y + Height; j ++)		//遍历指定页
	{
		for (i = X; i < X + Width; i ++)	//遍历指定列
		{
			...
			{
				OLED_DisplayBuf[j / 8][i] = 0;	//将显存数组指定数据清零
			}
		}
	}	
}
